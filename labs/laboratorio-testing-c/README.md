...
> R: carrito_init inicializa el carrito y establece su cantidad en 0, dejándolo vacío y en un estado conocido. Es importante llamarla antes de usarlo porque evita trabajar con valores no inicializados.
...
> R:El test falla, se esperaba 700 y obtuvimos 350
...
> R:El error esta en la linea 24

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R:La linea debe multiplicar el unitario por la cantidad de unidades del producto 
...
> R:El test falla, carrito_agregar deberia devolver 0 y esta devolviendo 1.
...
> R:El operador incorrecto era: <= pero ya lo modificamos a < para que todo funcione correctamente
...
> R: Si las lineas 29 y 30. No se ejecutaron porque los test utilizados para medir la cobertura no llamaron a esa funcion
...
> R: Un test unitario verifica una función o componente de forma aislada, mientras que un test de integración verifica que varias funciones trabajen correctamente en conjunto. El bug de carrito_total fue detectado primero por un test unitario, específicamente test_total_con_cantidad.
...
> R: En C los arrays no realizan un control automático de los límites. Si se intenta escribir fuera del límite del array, se puede sobrescribir memoria perteneciente a otras variables y provocar corrupción de datos, comportamiento indefinido o vulnerabilidades. En lenguajes como Python o Java se realizan controles de límites y se produce un error o excepción cuando se intenta acceder fuera del rango, evitando que se escriba directamente en memoria fuera del array.
...
> R: Escribir tests permite detectar los errores comprobando el comportamiento real del programa, en lugar de depender solamente de la lectura del código para encontrar posibles errores. Además, los tests permiten reproducir el problema, verificar que la corrección funciona y comprobar posteriormente que el error no vuelva a aparecer.
...
> R: El primer test no detectó el bug porque utilizaba una cantidad igual a 1. En ese caso, 350 × 1 = 350, por lo que aunque carrito_total ignorara la cantidad y solamente utilizara el precio, el resultado sería correcto. En cambio, con cantidad 2, el resultado esperado es 350 × 2 = 700, pero el código con el bug obtenía 350, por lo que el test fallaba.
